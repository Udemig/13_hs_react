      ...
      <tbody>
        {orders.map((order, key) => (
          <tr key={key}>
            ...
            <td className="text-green-600">{order.total_price.toFixed(2)} TL</td>

            <td>{order.shipping_address.city}</td>

            <td>
              <span className={`${getColor(order.status)} text-white py-1 px-2 rounded-lg shadow w-full text-center`}>
                {order.status}
              </span>
            </td>
          </tr>
